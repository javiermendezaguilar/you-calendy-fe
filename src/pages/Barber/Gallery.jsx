...
// Gallery component for barber profile - uses data from localStorage
...
const Gallery = () => {
  ...
  useEffect(() => {
    const fetchGalleryImages = async () => {
      try {
        ...
        // Check if this is a public barber profile
        ...
        if (publicBarberData && publicBusinessId) {
          // This is a public barber profile - use barber data
          const barberData = JSON.parse(publicBarberData);
          const galleryImages = barberData.business.profileImages?.galleryImages || [];
          
          if (galleryImages.length > 0) {
            // Transform barber gallery images to match our gallery format
            // galleryImages is an array of URL strings from the API
            const transformedImages = galleryImages.map((imageUrl, index) => ({
              id: index + 1,
              src: imageUrl, // imageUrl is already a string URL
          ...
          // No barber profile data found - set empty array
          setGalleryImages([]);
        }
  ...
  return (
    <motion.section 
      className="container p-3 md:p-5 lg:p-8 max-w-full min-h-[600px]"
      variants={containerVariants}
      initial="hidden"
      animate="visible"
    >
